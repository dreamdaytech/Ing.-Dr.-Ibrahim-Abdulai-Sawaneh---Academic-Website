...
const target1 = `      const htmlContent = \`
        <div style="font-family: Arial, sans-serif; color: #1e293b; max-width: 800px; margin: 0 auto; padding: 20px;">
          ...
          <h3 style="font-size: 20px; color: #0f172a; margin-bottom: 15px;">Book Abstract / Overview</h3>
          ...
          <div style="padding: 20px; background-color: #f8fafc; border-left: 4px solid #94a3b8; font-size: 14px; line-height: 1.6; color: #475569;">
            <em>Note: This is an automatically generated excerpt document for preview purposes. For the full volume, please consult academic retailers or institutional libraries.</em>
          </div>
        </div>
      \`;`;

const replacement1 = `      const htmlContent = \`
        <div style="font-family: Arial, sans-serif; color: #1e293b; max-width: 800px; margin: 0 auto; padding: 20px;">
          <h1 style="font-family: Georgia, serif; font-size: 28px; color: #0f172a; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 20px;">
            \${activeBookSelected.title}
          </h1>
...
const target2 = `{isExportingExcerpt ? 'Generating PDF...' : 'Download Excerpt Chapter'}`;
const replacement2 = `{isExportingExcerpt ? 'Generating PDF...' : 'Download Full Index & Synopsis'}`;

if (code.includes(target1) && code.includes(target2)) {
  code = code.replace(target1, replacement1);
  code = code.replace(target2, replacement2);
  fs.writeFileSync('src/components/BooksSection.tsx', code, 'utf8');
  console.log('Successfully patched BooksSection.tsx');
} else {
